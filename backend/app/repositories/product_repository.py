...
from app.models.variante_digital import VarianteDigital
...
class ProductRepository:
        ...
        try:
            ...
            if data.videojuego:
                ...
                if len(generos) != len(data.videojuego.generos_ids):
                    logger.warning(
                        "Algunos IDs de género proporcionados no existen."
                    )

                videojuego.generos = generos

                logger.debug(
                    "Información de videojuego y géneros vinculada"
                )
            ...
            logger.info(
                f"Producto '{producto.nombre}' creado exitosamente con ID: {producto.id}"
            )
            ...
            logger.error(
                f"Error crítico al crear producto: {str(e)}",
                exc_info=True
            )
    ...
    def get_all(self):
        ...
        try:
            ...
            return self.db.query(Producto).options(
                ...
                .joinedload(ProductoVariante.plataforma),

                joinedload(Producto.variantes)
                .joinedload(ProductoVariante.formato),

                joinedload(Producto.variantes)
                .joinedload(ProductoVariante.detalle_digital),

                joinedload(Producto.videojuego)
                .joinedload(Videojuego.generos)
            ...
            raise HTTPException(
                status_code=500,
                detail="Error al obtener la lista"
            )
    ...
    def get_by_id(self, producto_id: int):
        ...
        if not producto:
            ...
            logger.warning(
                f"Intento fallido de obtener producto inexistente: ID {producto_id}"
            )
    ...
    def delete(self, producto_id: int):
        ...
        try:
            ...
            if not producto:
                ...
                logger.warning(
                    f"Intento de eliminar producto inexistente: ID {producto_id}"
                )
            ...
            logger.info(
                f"Producto ID {producto_id} eliminado correctamente."
            )
        ...
        try:
            ...
            if precio_min is not None or precio_max is not None:
                ...
                if precio_min is not None:
                    condiciones.append(
                        ProductoVariante.precio >= precio_min
                    )

                if precio_max is not None:
                    condiciones.append(
                        ProductoVariante.precio <= precio_max
                    )
            ...
            logger.error(
                f"Error en la búsqueda de productos: {str(e)}"
            )
